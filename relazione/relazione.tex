...
% Evita che le righe lunghe sbordino dai margini: microtype riduce le
% righe con l'espansione dei font, l'emergencystretch assorbe i residui e
% i nomi di funzione in \texttt possono spezzarsi dopo l'underscore.
\setlength{\emergencystretch}{3em}
\makeatletter
\let\@origtexttt\texttt
\renewcommand{\texttt}[1]{{\def\_{\textunderscore\discretionary{}{}{}}\@origtexttt{#1}}}
\makeatother
...
% Stile per i listati Python: spezza le righe lunghe entro i margini e
% gestisce i caratteri accentati (UTF-8) presenti nei sorgenti.
\definecolor{codebg}{gray}{0.97}
\definecolor{breakarrow}{rgb}{0.55,0,0}
\lstdefinestyle{pyrelazione}{
  language=Python,
  ...
  breakindent=0pt,
  postbreak=\mbox{\textcolor{breakarrow}{$\hookrightarrow$}\space},
...
\vspace{2.2cm}
{\Huge\bfseries Progetto 2\par}
\vspace{0.6cm}
{\Large Compressione di immagini in scala di grigio con DCT\par}
...
L'obiettivo di questo progetto è studiare e applicare la Trasformata Discreta del Coseno
(DCT) per la compressione di immagini in toni di grigio, riproducendo il principio alla base dello
standard JPEG. Il lavoro si articola in due parti.

Nella prima parte abbiamo implementato la trasformata bidimensionale: abbiamo scritto
una versione basata sul prodotto matriciale e l'abbiamo confrontata con l'implementazione
\textit{fast} basata su FFT offerta dalle librerie scientifiche, misurandone i tempi di
esecuzione e verificandone la correttezza numerica.

Nella seconda parte abbiamo sviluppato un software con interfaccia grafica che applica
l'algoritmo. Il programma permette di caricare immagini dal \textit{filesystem}, suddividerle
in blocchi e osservare gli effetti visivi della compressione. In particolare, abbiamo
analizzato il compromesso tra livello di compressione e perdita di qualità al variare di
due parametri: la dimensione dei blocchi $F$ e la soglia di taglio delle alte frequenze $d$.

\subsection{Linguaggio e dipendenze}

Il progetto è interamente scritto in Python 3.12. Tutti i pacchetti
...
\section{Parte 1: implementazione e confronto della DCT2}

\subsection{DCT2}

Partendo dalla teoria delle serie di Fourier, la Trasformata Discreta del Coseno (DCT) nasce dall'esigenza
di approssimare un segnale campionato riducendo il fenomeno di Gibbs ai bordi.
L'idea è \emph{riflettere} la funzione rispetto all'asse delle ordinate, rendendola pari,
per poi periodizzarla: in questo modo si eliminano le discontinuità artificiali
generate da una semplice replicazione e si azzerano i coefficienti associati ai seni,
lasciando una base di soli coseni ortogonali.

Un principio centrale che rende la DCT adatta alla compressione JPEG è il seguente: più la funzione,
o l'immagine, è "liscia", più i coefficienti della trasformata decadono rapidamente all'aumentare delle frequenze.
Poiché le immagini naturali presentano solitamente variazioni di intensità graduali, la DCT concentra quasi tutta
l'energia in pochi coefficienti a bassa frequenza. Questo permette di azzerare
le alte frequenze introducendo un degrado visivo minimo o nullo.

Per estendere questo concetto al caso bidimensionale, abbiamo sfruttato la proprietà di \emph{separabilità}
del nucleo coseno. Nella nostra implementazione \emph{custom} (\texttt{dct\_2d\_homemade}) abbiamo costruito
esplicitamente la matrice ortonormale di trasformazione $D \in \mathbb{R}^{N \times N}$, riga per riga,
pesando i coseni per i rispettivi coefficienti di normalizzazione:
\[
...
la trasformata prima alle colonne e poi alle righe, il che equivale al doppio prodotto matriciale
$D \cdot f \cdot D^\top$, calcolato in \texttt{numpy} con l'operatore \texttt{@}. Trattandosi di due
moltiplicazioni dense tra matrici, il costo asintotico di questa procedura è $O(N^3)$.

Per la versione di libreria abbiamo invece usato le funzioni \texttt{scipy.fft.dctn} e \texttt{scipy.fft.idctn}.
Questa implementazione sfrutta internamente l'algoritmo FFT (\textit{Fast Fourier Transform}),
che riduce il costo computazionale a $O(N^2 \log N)$.
Le funzioni sono state richiamate con i parametri \texttt{type=2} e
\texttt{norm="ortho"}, in modo da usare la stessa base ortonormale appena descritta e rendere
confrontabili i risultati delle due implementazioni.
...
\begin{itemize}
  \item \emph{custom} vs \emph{fast}: le due
        implementazioni coincidono a meno della precisione di macchina, con errori
        dell'ordine di $10^{-13}$, ampiamente entro la soglia $10^{-10}$ usata
        nei test;
...
\midrule
8    & 2.90e-05 & 8.50e-06 & 3.4  \\
16   & 4.45e-05 & 5.67e-06 & 7.9  \\
32   & 8.04e-05 & 9.75e-06 & 8.2  \\
64   & 1.62e-04 & 1.48e-05 & 10.9 \\
128  & 3.40e-04 & 4.45e-05 & 7.6  \\
256  & 8.86e-04 & 2.21e-04 & 4.0  \\
512  & 3.41e-03 & 1.14e-03 & 3.0  \\
768  & 7.46e-03 & 2.42e-03 & 3.1  \\
1024 & 1.55e-02 & 5.33e-03 & 2.9  \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Parte 2: compressione di immagini}

\subsection{L'algoritmo di compressione}

L'algoritmo di compressione che abbiamo sviluppato, ispirato alla logica di base del formato JPEG, si articola in questi passaggi:
\begin{enumerate}
    \item \textbf{Suddivisione in blocchi:} Dividiamo l'immagine originale in blocchi quadrati di dimensione fissa $F \times F$, partendo dall'angolo
    in alto a sinistra. Scartiamo le eventuali righe o colonne in avanzo sul bordo destro e su quello inferiore, che non permettono di formare un blocco
    completo.
    ...
    \item \textbf{Arrotondamento e composizione:} A causa della perdita introdotta dal taglio, i pixel ricostruiti possono uscire dai limiti della scala
    di grigi. Li arrotondiamo perciò all'intero più vicino, li limitiamo all'intervallo fra 0 e 255 e infine li ricomponiamo nella posizione originaria per formare l'immagine compressa.
\end{enumerate}
...
\begin{itemize}
      ...
      \item Per azzerare le alte frequenze abbiamo evitato cicli espliciti, pre-calcolando un'unica maschera booleana applicata all'intero blocco tramite l'indicizzazione di \texttt{numpy} (\texttt{coefficients[\textasciitilde mask] = 0.0}).
      \item Infine, abbiamo combinato le funzioni \texttt{np.rint} e \texttt{np.clip} per arrotondare i valori ricostruiti all'intero più vicino e limitarli all'intervallo dei pixel ($0$--$255$).
\end{itemize}

\subsection{Interfaccia grafica}

Per interagire con il codice e provare la compressione abbiamo realizzato un'interfaccia grafica (\texttt{gui.py}) con la libreria \texttt{tkinter}.

Il programma permette di caricare un file \texttt{.bmp} a scelta e di
variare i parametri $F$ e $d$. Gli input inseriti vengono validati: la
dimensione del blocco deve essere positiva ($F > 0$) e non può superare la risoluzione dell'immagine, e
la soglia di taglio deve rispettare il vincolo $0 \le d \le 2F-2$ prima di avviare l'elaborazione.
Sotto le caselle di input compaiono inoltre suggerimenti aggiornati in tempo reale sui valori ammissibili di $F$ e $d$.
...
Per interpretare i risultati sperimentali ci siamo basati su due principi teorici.

Il primo è la \emph{concentrazione dell'energia}: in un segnale sufficientemente regolare o
liscio, i coefficienti della DCT decadono in modo rapido. Di conseguenza, quasi tutta l'energia del segnale
si concentra nelle basse frequenze, localizzate nell'angolo in alto a sinistra della matrice del blocco.
Poiché la trasformazione utilizzata è ortonormale, l'energia totale si conserva: pertanto,
l'azzeramento delle alte frequenze comporta una perdita trascurabile di
informazione visiva.

Il secondo principio riguarda le discontinuità. Un \emph{bordo netto} all'interno dell'immagine richiede,
per essere rappresentato fedelmente, un contributo significativo delle alte frequenze.
Troncando la serie e azzerando tali frequenze si introduce il \emph{fenomeno di Gibbs}, che si manifesta
con ondulazioni attorno al bordo e un effetto di smussamento.

Per la valutazione della qualità di ricostruzione abbiamo adottato come metrica di riferimento il
PSNR (Peak Signal-to-Noise Ratio).
Per interpretare i risultati seguiamo queste soglie di valutazione:
\begin{itemize}
...
Il caso del gradiente rappresenta lo scenario ideale per la DCT.
Poiché l'intensità luminosa varia in modo graduale lungo un'unica direzione,
il segnale è molto regolare. Applicando la trasformata a un blocco così uniforme
si ottiene uno spettro fortemente concentrato: quasi tutta
l'informazione si trova nel coefficiente $c_{0,0}$, che rappresenta la luminosità
media, e nel primo coefficiente direzionale. Di conseguenza,
...
I risultati numerici e l'osservazione visiva confermano questa ipotesi.
Mantenendo solo i primi coefficienti, con soglia $d=2$ e quindi meno del $5\%$ dei
coefficienti totali, il PSNR supera i $54$~dB e la ricostruzione è visivamente
indistinguibile dall'originale.

Aumentare ulteriormente il parametro $d$ non porta miglioramenti di rilievo,
perché i pochi coefficienti necessari a descrivere il gradiente sono già stati inclusi:
con le sole primissime frequenze l'errore di ricostruzione di fatto si annulla.
Questo comportamento conferma l'ipotesi di partenza: le porzioni
di immagine con variazioni di intensità graduali vengono
compresse in modo ottimale dalla DCT, e si può azzerare la grande maggioranza dei
coefficienti senza perdita di qualità percepibile.

\subsubsection{Carattere a contrasto pieno (\texttt{prova})}

L'immagine di test \texttt{prova}, che raffigura una lettera $C$ chiara su sfondo scuro, presenta
pochi bordi netti ad alto contrasto ed è priva di dettagli e texture fini.
È lo scenario opposto rispetto a quello del gradiente e permette di osservare
il fenomeno di Gibbs. Poiché i bordi sono discontinuità,
la loro informazione non è concentrata, ma distribuita su un ampio spettro di frequenze.
Di conseguenza, l'azzeramento delle frequenze più alte genera distorsioni visive
...
Con una soglia di taglio bassa, $d$ tra $2$ e $8$, il PSNR resta
tra i $17$ e i $27$~dB e l'ispezione visiva della griglia evidenzia un degrado marcato.
Le sole basse frequenze non approssimano bene il perimetro della lettera,
e attorno alla figura compare un evidente alone chiaro.
Solo portando $d$ verso i valori massimi si torna a una
qualità visiva soddisfacente, perché si reintroducono i coefficienti necessari a descrivere le transizioni nette.

Un aspetto interessante riguarda la dimensione del blocco:
a parità di percentuale di coefficienti conservati, i blocchi più ampi danno risultati migliori.
Ad esempio, con $F=32$ e $d=40$, che mantiene circa il $73\%$ dei coefficienti, si raggiungono i $31$~dB, contro
i $27$~dB ottenuti con $F=8$ e $d=8$, che ne trattiene circa il $56\%$. Questo accade
perché un blocco più esteso riesce a catturare l'andamento
del contorno su una porzione più ampia dell'immagine.

\subsubsection{Scacchiera}

L'immagine della scacchiera è il caso peggiore per un algoritmo di
compressione a blocchi basato sulla DCT.
Il motivo sta nella natura del segnale: è un pattern periodico di celle nere e bianche
da $10 \times 10$ pixel, con transizioni nette da $0$ a $255$ a ogni bordo di cella.
Questi bordi, ripetuti su tutta l'immagine, spostano gran parte dell'energia
nelle alte frequenze. Poiché la maschera di filtraggio impone la condizione $k+\ell < d$,
queste componenti sono le prime a essere scartate.
In altri termini, l'informazione essenziale dell'immagine si trova proprio dove il
compressore interviene. È la situazione opposta
...
Per analizzare l'interazione tra questo fenomeno e la dimensione del blocco,
abbiamo condotto test su risoluzioni crescenti, da $20 \times 20$ fino a $640 \times 640$ pixel.
Poiché le celle della scacchiera misurano sempre $10 \times 10$ pixel, cambiare la risoluzione
modifica il numero di celle incluse in ogni blocco $F \times F$ e, di
conseguenza, la capacità della DCT di concentrare l'energia.
...
\caption{Scacchiera, PSNR (dB) per tutte le risoluzioni testate,
con $F=8$ e $F=32$. Le colonne sono il lato dell'immagine in pixel.
...
I dati numerici e l'ispezione visiva danno un quadro coerente. Con una soglia di taglio bassa il
PSNR crolla sotto i $18$~dB.
Senza il suo contenuto ad alta frequenza, la scacchiera subisce un degrado severo: di ogni blocco
resta solo la media di intensità, e l'immagine diventa una superficie di tasselli grigi,
come si vede nei pannelli in alto a sinistra delle griglie di ricostruzione.
La struttura geometrica originaria riemerge solo con valori di $d$ elevati,
cioè conservando quasi tutti i coefficienti e
rinunciando a una compressione significativa.

Il confronto tra le diverse risoluzioni mostra un comportamento interessante: a parità di soglia $d$,
le metriche risultano identiche, ma solo per alcune combinazioni di risoluzione e dimensione
del blocco. Con $F=8$,
le metriche coincidono per tutte le risoluzioni esaminate, tranne la scacchiera
più piccola da $20 \times 20$.
Con un blocco più grande, ad esempio $F=32$,
l'equivalenza non vale più per le risoluzioni intermedie come la $40 \times 40$ o la $80 \times 80$,
ma torna a manifestarsi per le griglie più ampie come la $160 \times 160$ e la $640 \times 640$.

La nostra intuizione è che questo fenomeno dipenda da un allineamento
geometrico. Poiché la scacchiera ha celle da
$10 \times 10$ pixel, la sua struttura e la griglia dei blocchi di analisi
si riallineano a intervalli regolari. Se la
risoluzione dell'immagine è un multiplo esatto di questo intervallo,
l'algoritmo elabora più volte le stesse sequenze di
pixel. Di conseguenza, l'insieme dei blocchi distinti rimane invariato e
i valori di PSNR coincidono.

Ad esempio, con $F=8$ la griglia dei blocchi e quella della
scacchiera si riallineano ogni 40 pixel. Per questo
...
Un caso particolare e limite si osserva impostando la dimensione del blocco $F=10$. Essendo le celle della scacchiera esattamente di $10 \times 10$ pixel, questa scelta garantisce un allineamento spaziale perfetto tra la partizione a blocchi dell'algoritmo e la struttura dell'immagine. In questa specifica configurazione ogni blocco ricade interamente all'interno di una singola cella, contenendo pixel di intensità uniforme. L'intera energia spaziale si concentra così nella sola componente continua della DCT, permettendo all'algoritmo di raggiungere una compressione perfetta a prescindere dal parametro $d$, a patto di mantenerlo strettamente positivo (come mostrato nei test seguenti con $d=1$).

\begin{figure}[H]
\centering
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/20x20_grid_F10.png}
    \caption{$20\times20$}
\end{subfigure}
\hfill
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/160x160_grid_F10.png}
    \caption{$160\times160$}
\end{subfigure}
\hfill
\begin{subfigure}{0.32\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/640x640_grid_F10.png}
    \caption{$640\times640$}
\end{subfigure}
\caption{Compressione perfetta della scacchiera con $F=10$ ($d=1$).}
\label{fig:scacchiere-f10}
\end{figure}

\subsection{Fotografie reali}

Le cinque fotografie analizzate (\texttt{bridge}, \texttt{cathedral},
\texttt{deer}, \texttt{shoe}, \texttt{tonypitony}) sono i nostri casi
d'uso reali: contengono texture naturali, gradienti morbidi e
bordi netti. Di conseguenza, presentano
una comprimibilità intermedia: nettamente superiore alla scacchiera,
ma inferiore al gradiente puro. Da un punto di vista teorico, il
fattore determinante è che le immagini naturali sono localmente lisce.
All'interno di un blocco di dimensioni ridotte $F \times F$ le variazioni
di intensità sono per lo più graduali, e questa regolarità permette alla DCT
di concentrare l'energia del segnale in pochi coefficienti a
bassa frequenza.
...
un quarto dei coefficienti, il PSNR si attesta tra i $31$ e i $39$~dB, quindi una qualità visiva buona.

Le differenze tra le cinque immagini riflettono il loro contenuto visivo.
La foto \texttt{cathedral}, grazie alle ampie superfici
uniformi di cielo e muratura, è la più comprimibile. Al contrario,
immagini come \texttt{shoe} e \texttt{tonypitony}, ricche di texture e
dettagli fini, hanno metriche inferiori alle basse soglie, ma migliorano
rapidamente al crescere di $d$. Questo accade perché
...
comprimibilità che rispetta la predizione teorica:
...
\begin{itemize}
  \item \textbf{Segnale liscio} (gradiente): spettro concentrato in pochi
        ...
        ricostruzione senza perdita molto prima del massimo. La DCT è lo strumento ideale.
  \item \textbf{Discontinuità sparse} (fotografie reali, carattere
        \texttt{prova}): bordi netti ma isolati. Compressione utile già a
        $d=5$--$8$ e qualità quasi senza perdita a $d$ alto. È il caso d'uso
        reale del JPEG.
...
quadro. Il \emph{ringing} dovuto al fenomeno di Gibbs compare sui bordi netti a bassa $d$, come
l'alone attorno alla C o ai contorni delle foto: è l'effetto del troncamento
...
specialmente a $d$ piccolo: ogni blocco è elaborato in modo
indipendente e i suoi bordi possono non combaciare dopo la ricostruzione, quindi
sull'immagine si vede una griglia sovrapposta. Blocchi più grandi rendono
questa griglia più vistosa, perché le discontinuità coprono aree più estese, ma
concentrano meglio l'energia; blocchi più piccoli confinano l'artefatto ma
comprimono peggio. Il compromesso $F=8$ dello standard JPEG sta
in mezzo, e i nostri esperimenti ne mostrano il motivo.
...
DCT2 e IDCT2 fast via \texttt{scipy.\allowbreak fft.\allowbreak dctn}/\allowbreak\texttt{idctn} con
...
Di seguito si riporta il codice sorgente completo dei moduli del
progetto, la cui documentazione è descritta nella sezione precedente.

\subsection{\texttt{dct\_utils.py}}
\lstinputlisting[language=Python]{../src/dct_utils.py}

\subsection{\texttt{run\_assignment.py}}
\lstinputlisting[language=Python]{../src/run_assignment.py}

\subsection{\texttt{gui.py}}
\lstinputlisting[language=Python]{../src/gui.py}

\subsection{\texttt{test\_assignment.py}}
\lstinputlisting[language=Python]{../src/test_assignment.py}
